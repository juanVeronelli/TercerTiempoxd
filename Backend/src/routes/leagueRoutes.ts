...
import {
  ...
  updateMemberRole,
  getAdvancedStats,
  getMonthlyPredictionRewardPopup,
...
import {
  ...
  updateScheduledRule,
  deleteScheduledRule,
} from "../controllers/scheduledMatchRuleController.js";
import { authenticateToken } from "../middlewares/authMiddleware.js";
import { createLogger } from "../utils/logger.js";

const log = createLogger("leagueRoutes");
import { validateRequest } from "../middlewares/validationMiddleware.js";
import { createScheduledRuleSchema, updateScheduledRuleSchema } from "../schemas/scheduledRuleSchemas.js";
...
router.get("/:id/monthly-reward-popup", authenticateToken, getMonthlyPredictionRewardPopup);
// Reglas de partidos programados
router.get("/:leagueId/scheduled-rules", authenticateToken, listScheduledRules);
router.get("/:leagueId/scheduled-rules/:ruleId", authenticateToken, getScheduledRule);
router.post(
  "/:leagueId/scheduled-rules",
  authenticateToken,
  validateRequest(createScheduledRuleSchema),
  createScheduledRule,
);
router.put(
  "/:leagueId/scheduled-rules/:ruleId",
  authenticateToken,
  validateRequest(updateScheduledRuleSchema),
  updateScheduledRule,
);
router.delete("/:leagueId/scheduled-rules/:ruleId", authenticateToken, deleteScheduledRule);
router.get(
  ...
  (req, res, next) => {
    uploadLeaguePhotoMulter.single("photo")(req, res, (err: unknown) => {
      if (err) {
        ...
        log.errorWithErr("multer upload league photo failed", err);
