...
const log = createLogger("DuelService");
...
export async function generateMatchDuel(matchId: string): Promise<{
  ...
  if (!match) {
    throw new DomainError({ status: 404, code: "PARTIDO_NO_ENCONTRADO" });
  }
  ...
  if (existingDuel) {
    throw new DomainError({ status: 400, code: "YA_EXISTE_DUELO" });
  }
  ...
  if (confirmedPlayerIds.length < 2) {
    throw new DomainError({ status: 400, code: "SE_NECESITAN_2_JUGADORES_CONFIRMADOS" });
  }

  if (!match.league_id) {
    throw new DomainError({ status: 400, code: "PARTIDO_SIN_LIGA" });
  }

  const leagueId = match.league_id;

  // Consumible: slot asegurado en duelo (duel_reserved_slot).
  // Si existe activación PRE_MATCH para este partido, el usuario forzado debe estar en el duelo.
  const reservedActs = await prisma.user_consumable_activations.findMany({
    where: {
  ...
  const reservedKeys = [...new Set(reservedActs.map((a) => a.consumable_key))];
  const reservedRows = reservedKeys.length
    ? await prisma.shop_items.findMany({
        where: { consumable_key: { in: reservedKeys }, item_type: "CONSUMABLE" },
  ...
  const reservedUserIds = reservedActs
    .filter((a) => (effectByKey.get(a.consumable_key) ?? a.consumable_key) === "duel_reserved_slot")
    .map((a) => a.user_id)
    .filter((uid) => confirmedPlayerIds.includes(uid));
  const forcedUserId =
    reservedUserIds.length > 0 ? [...reservedUserIds].sort((a, b) => a.localeCompare(b))[0]! : null;
  ...
  if (members.length < 2) {
    throw new DomainError({ status: 400, code: "DATOS_INSUFICIENTES_MIEMBROS" });
  }
  ...
  if (candidates.length === 0) {
    throw new DomainError({ status: 400, code: "NO_PAREJAS_COMPATIBLES" });
  }

  // Ordenar por diff (más equilibrados primero) para tener buen pool
  candidates.sort((a, b) => a.diff - b.diff);

  let selectedPair: CandidatePair | undefined;
  if (forcedUserId) {
    const forcedCandidates = candidates.filter(
      (c) => c.p1.user_id === forcedUserId || c.p2.user_id === forcedUserId,
    );
    if (forcedCandidates.length === 0) {
      throw new DomainError({ status: 400, code: "NO_PAREJAS_COMPATIBLES_FORZADAS" });
    }
    const differentTeamForced = forcedCandidates.filter((c) => !c.sameTeam);
    const poolForced = differentTeamForced.length > 0 ? differentTeamForced : forcedCandidates;
    const topN = Math.min(poolForced.length, 6);
    const poolSlice = poolForced.slice(0, topN);
    selectedPair = poolSlice[Math.floor(Math.random() * poolSlice.length)];
  } else {
    ...
    const pool = differentTeam.length > 0 ? differentTeam : sameTeam;
  ...
  if (!selectedPair?.p1?.user_id || !selectedPair?.p2?.user_id) {
    throw new DomainError({ status: 500, code: "ERROR_SELECCION_PAREJA" });
  }
  ...
  // Consumir activación del slot asegurado si se usó.
  if (forcedUserId) {
    const usedActs = reservedActs.filter(
      (a) =>
        a.user_id === forcedUserId &&
        (effectByKey.get(a.consumable_key) ?? a.consumable_key) === "duel_reserved_slot",
    );
    if (usedActs.length) {
      await prisma.user_consumable_activations.updateMany({
        where: { id: { in: usedActs.map((a) => a.id) } },
...
export async function resolveMatchDuel(
  ...
  try {
    ...
    if (ratingA > ratingB) {
      ...
      // Empate: no dejar sin ganador. Desempate estable por id (evita "no lo entrega").
      winnerId =
        String(duel.challenger_id).localeCompare(String(duel.rival_id)) <= 0
          ? duel.challenger_id
          : duel.rival_id;
    }
    ...
    log.errorWithErr("Error resolviendo duelo", error);
    return undefined;
  }
}
