...
function getResend(): Resend | null {
  ...
  if (!key) {
    log.warn("RESEND_API_KEY no está definido. Los emails no se enviarán.");
    return null;
  }
...
export async function sendVerificationEmail(email: string, code: string) {
  ...
  try {
    ...
    const err = res?.error;
    if (err) {
      log.error("Verificación rechazada", { message: err.message, email });
    }
  } catch (err) {
    log.errorWithErr("Error enviando verificación", err, { email });
  }
}
...
export async function sendPasswordResetEmail(email: string, token: string) {
  ...
  try {
    ...
    const err = res?.error;
    if (err) {
      log.error("Reset rechazado", { message: err.message, email });
    }
  } catch (err) {
    log.errorWithErr("Error enviando reset password", err, { email });
  }
}
