...
export type PlayerForAccordion = {
  ...
  consumableIcon?: keyof typeof Ionicons.glyphMap;
    ...
    ({ item }: { item: PlayerForAccordion }) => (
      <View style={styles.chip}>
        <View style={styles.avatarWrap}>
          <UserAvatar imageUrl={item.photo} name={item.name} size={36} />
          {item.consumableKey ? (
            <View
              style={[
                styles.consumableBadge,
                { borderColor: item.consumableColor ?? "#F59E0B" },
              ]}
            >
              <Ionicons
                name={item.consumableIcon ?? "flash"}
                size={10}
                color={item.consumableColor ?? "#F59E0B"}
              />
            </View>
          ) : null}
        </View>
