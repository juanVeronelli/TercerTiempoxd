...
export const NextMatchCard = ({
  ...
  const userStatusRaw = match.user_status;
  const userStatus =
    typeof userStatusRaw === "string" ? userStatusRaw.toUpperCase() : "";

  const isSummoned =
    userStatus === USER_STATUS_CONFIRMED ||
    userStatus === USER_STATUS_PENDING;

  const spectatorAttending = match?.spectator_attending === true;
  const isOpenSignup = match?.is_open_signup === true;
  const maxPlayers = typeof match?.max_players === "number" ? match.max_players : null;
  const signedUpCount = typeof match?.signed_up_count === "number" ? match.signed_up_count : null;
  const userSignedUp = match?.user_signed_up === true;
  const isFull = typeof maxPlayers === "number" && typeof signedUpCount === "number" && signedUpCount >= maxPlayers;

  // 1. ESTADO DEL USUARIO
  const isConfirmed =
    match.has_confirmed === true ||
    userStatus === USER_STATUS_CONFIRMED;

  // 2. PERMISOS DE INTERACCIÓN
  // Solo convocados pueden confirmar/cancelar, solo antes del horario del partido, y solo si OPEN.
  const isMatchOpen = match.status === "OPEN";
  const isVoting = match.status === "FINISHED";
  const kickoffMs = dateObj.getTime();
  const isBeforeKickoff =
    Number.isFinite(kickoffMs) && Date.now() < kickoffMs;
  const canInteract = isMatchOpen && isSummoned && isBeforeKickoff;
  ...
  return (
    <View style={styles.card}>
      ...
      <View style={styles.footer}>
        {isVoting && onVote ? (
          <TouchableOpacity
            style={[styles.confirmButton, { backgroundColor: "#F59E0B" }]}
            onPress={onVote}
            activeOpacity={0.85}
          >
            <Text style={styles.confirmText}>VOTAR AHORA</Text>
            <Ionicons
              name="star"
              size={20}
              color="white"
          ...
          <View>
            ...
            {userSignedUp ? (
              <View style={styles.statusBoxConfirmed}>
                <View style={{ flexDirection: "row", alignItems: "center" }}>
                  <Ionicons name="checkmark-done" size={20} color="#4ADE80" />
                  <Text style={styles.confirmedText}>YA ESTÁS ANOTADO</Text>
                </View>
                {onUnsignup ? (
                  <TouchableOpacity
                    ...
                    activeOpacity={0.8}
                  >
                    <Text style={styles.secondaryActionText}>DESANOTARME</Text>
                  </TouchableOpacity>
                ) : null}
              </View>
            ) : onSignup ? (
              <TouchableOpacity
                style={[styles.confirmButton, isFull && { opacity: 0.45 }]}
                onPress={isFull ? undefined : onSignup}
                activeOpacity={isFull ? 1 : 0.85}
              >
                <Text style={styles.confirmText}>
                  {isFull ? "CUPO COMPLETO" : "ANOTARME"}
                </Text>
                <Ionicons
                  name="add-circle"
                  size={20}
          ...
          spectatorAttending ? (
            <View style={styles.statusBoxConfirmed}>
              <View style={{ flexDirection: "row", alignItems: "center" }}>
                <Ionicons name="eye" size={20} color="#4ADE80" />
                <Text style={styles.confirmedText}>VAS COMO ESPECTADOR</Text>
              </View>
              {onUnspectate ? (
                <TouchableOpacity
                  style={styles.secondaryActionBtn}
                  onPress={onUnspectate}
                  activeOpacity={0.8}
                >
            ...
            <TouchableOpacity
              ...
              activeOpacity={0.85}
            >
              <Text style={styles.confirmText}>VOY A IR COMO ESPECTADOR</Text>
              <Ionicons
                name="eye"
                size={20}
                color="white"
            ...
            <View style={styles.statusBoxNone}>
              ...
              <Text style={styles.noneText}>
                No estás en la convocatoria de este partido
              </Text>
            </View>
          )
        ) : !isMatchOpen ? (
          ...
          <View style={styles.statusBoxNone}>
            ...
            <Text style={styles.noneText}>
              Ya pasó el horario del partido. No podés confirmar asistencia.
            </Text>
          </View>
        ) : (
