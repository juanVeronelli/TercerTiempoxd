...
export default function PublicProfileScreen() {
  ...
  const rawParams = useLocalSearchParams<{ id?: string | string[]; leagueId?: string | string[] }>();
  const id = typeof rawParams.id === "string" ? rawParams.id : rawParams.id?.[0];
  const leagueIdFromRoute =
    typeof rawParams.leagueId === "string" ? rawParams.leagueId : rawParams.leagueId?.[0];
  ...
  const fetchData = async () => {
    ...
    try {
      ...
      let dates: string[] = Array.isArray(data.activityDates) ? data.activityDates : [];
      if (leagueIdFromRoute && rawUser.id) {
        try {
          const statsRes = await apiClient.get(
            `/leagues/${leagueIdFromRoute}/users/${rawUser.id}/stats`,
          );
          if (Array.isArray(statsRes.data?.activityDates)) {
            dates = statsRes.data.activityDates;
          }
        } catch {
          // mantener actividad global del perfil
        }
      }
      ...
      showAlert(
        "Error",
        formatUserFacingError(error, "No se pudo cargar el perfil."),
        undefined,
        "error",
      );
      router.back();
