...
export default function MatchScreen() {
      ...
      if (status === 429) {
        showAlert(
          ...
          formatUserFacingError(
            error,
            "Has hecho muchas peticiones. Esperá un momento y deslizá para actualizar.",
          ),
          [{ text: "Entendido" }],
        );
      } else {
        showAlert(
          "Error",
          formatUserFacingError(error, "No se pudo cargar la sección de partidos."),
          undefined,
          "error",
        );
      }
  ...
  const handleSpectate = async (matchId: string) => {
    const targetId = matchId || nextMatch?.id;
    try {
      await apiClient.post(`/match/${targetId}/spectate`, {});
  ...
  const handleUnspectate = async (matchId: string) => {
    const targetId = matchId || nextMatch?.id;
    try {
      await apiClient.post(`/match/${targetId}/unspectate`, {});
  ...
  const handleSignup = async (matchId: string) => {
    const targetId = matchId || nextMatch?.id;
    try {
      await apiClient.post(`/match/${targetId}/signup`, {});
  ...
  const handleUnsignup = async (matchId: string) => {
    const targetId = matchId || nextMatch?.id;
    try {
      await apiClient.post(`/match/${targetId}/unsignup`, {});
      ...
      { text: "No", style: "cancel" },
      {
        ...
        onPress: async () => {
          ...
          try {
            ...
            showAlert(
              "Error",
              formatUserFacingError(e, "No se pudo cancelar."),
              undefined,
              "error",
            );
          }
            ...
            {isAdmin ? (
              ...
              >
                <View style={{ gap: 12 }}>
                  ...
                  >
                    <View style={styles.heroContent}>
                      <View style={styles.adminIconBox}>
                        <Ionicons name="add" size={22} color={THEME.accentBlue} />
                      </View>
                      <View style={{ flex: 1 }}>
                        <Text
                  ...
                  <TouchableOpacity
                    style={[styles.heroCardAdmin, styles.heroCardAdminSecondary]}
                    onPress={() =>
                      router.push({
                        pathname: "/(main)/league/match/scheduled-rules",
                        params: { leagueId },
                      })
                    }
                    activeOpacity={0.9}
                  >
                    <View style={styles.heroContent}>
                      <View style={[styles.adminIconBox, styles.adminIconBoxSecondary]}>
                        <Ionicons name="repeat" size={18} color={THEME.accentBlue} />
                      </View>
                      <View style={{ flex: 1 }}>
                        <Text style={[styles.heroTitle, { color: THEME.accentBlue }]}>
                          REGLAS AUTOMÁTICAS
                        </Text>
                        <Text style={styles.heroSubtitle}>
                          Crea partidos recurrentes (ej. todos los sábados).
                        </Text>
                      </View>
                      <Ionicons
                        name="chevron-forward"
            ...
            >
              ...
              {isAdmin ? (
                  ...
                  adminMatches.map((match) => (
                    <View key={match.id} style={{ marginBottom: 15 }}>
                      <NextMatchCard
                        ...
                        onSpectate={() => handleSpectate(match.id)}
                        onUnspectate={() => handleUnspectate(match.id)}
                        onSignup={() => handleSignup(match.id)}
                        onUnsignup={() => handleUnsignup(match.id)}
                        onVote={() =>
                          router.push({
                            pathname: "/(main)/league/match/vote",
                            params: { matchId: match.id },
                ...
                <NextMatchCard
                  ...
                  onSpectate={() => handleSpectate(nextMatch.id)}
                  onUnspectate={() => handleUnspectate(nextMatch.id)}
                  onSignup={() => handleSignup(nextMatch.id)}
                  onUnsignup={() => handleUnsignup(nextMatch.id)}
                  onVote={() =>
                    router.push({
                      pathname: "/(main)/league/match/vote",
...
const styles = StyleSheet.create({
  ...
  heroSubtitle: { color: THEME.textSecondary, fontSize: 11, lineHeight: 14 },
  adminWatermark: {
    position: "absolute",
    right: -30,
    bottom: -30,
