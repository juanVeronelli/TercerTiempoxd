...
const createSchema = z.object({
  ...
  maxPlayers: z
    ...
    .refine((n) => n == null || n >= 2, "El mínimo es 2"),
});
    ...
    <>
      ...
      <AppCard style={styles.cardForm}>
        ...
        <View style={styles.inputGroup}>
          ...
          <View style={styles.toggleContainer}>
            <TouchableOpacity
              style={[styles.toggleBtn, !form.isOpenSignup && styles.toggleBtnActive]}
              onPress={() => updateForm({ isOpenSignup: false })}
            >
              <Text style={[styles.toggleText, !form.isOpenSignup && { color: "white" }]}>
                CONVOCATORIA
              </Text>
            </TouchableOpacity>
            <TouchableOpacity
              style={[styles.toggleBtn, form.isOpenSignup && styles.toggleBtnActive]}
              onPress={() => updateForm({ isOpenSignup: true })}
            >
              <Text style={[styles.toggleText, form.isOpenSignup && { color: "white" }]}>
                ANOTACIÓN ABIERTA
              </Text>
        ...
        {form.isOpenSignup && (
          <View style={styles.inputGroup}>
            <Text style={styles.label}>LÍMITE DE JUGADORES</Text>
            <View style={styles.inputContainer}>
              <Ionicons name="people-outline" size={20} color="#9CA3AF" style={styles.inputIcon} />
              <TextInput
                style={styles.input}
                value={form.maxPlayers}
                onChangeText={(t) => updateForm({ maxPlayers: t })}
                placeholder="Ej: 10"
                placeholderTextColor="#4B5563"
                keyboardType="number-pad"
                onBlur={() => setTouched((p) => ({ ...p, maxPlayers: true }))}
              />
            </View>
            {touched.maxPlayers && errors.maxPlayers ? (
              <Text style={styles.helperError}>{errors.maxPlayers}</Text>
            ) : (
              <Text style={styles.legendText}>
                Cuando se complete el cupo, nadie más podrá anotarse.
              </Text>
            )}
          </View>
        )}
...
export default function CreateMatchScreen() {
  ...
  const validation = useMemo(() => {
    const result = createSchema.safeParse({
      ...
      price: form.price,
      isOpenSignup: form.isOpenSignup,
        ...
        if (st === 429) {
          showAlert(
            ...
            formatUserFacingError(
              err,
              "Esperá un momento y volvé a entrar a esta pantalla.",
            ),
            [{ text: "Entendido" }],
          );
        } else {
          showAlert(
            "Error",
            formatUserFacingError(err, "No se pudo cargar el plantel."),
            [
              {
                text: "Reintentar",
  ...
  const handleCreateMatch = async () => {
    ...
    if (!form.isOpenSignup && selectedPlayers.length === 0) {
      showAlert("Atención", "Debes convocar al menos un jugador.");
      return;
    }
    if (form.isOpenSignup && (!form.maxPlayers || Number(form.maxPlayers) < 2)) {
      showAlert("Atención", "Debes definir un límite de jugadores (mínimo 2).");
      return;
    }
    ...
    try {
      ...
      const playersPayload = form.isOpenSignup
        ? []
        : selectedPlayers.map((p) => ({
            id: p.id,
            team: p.status === "LOCAL" ? "A" : "B",
          }));
      // En React Native, axios `timeout` puede no abortar realmente. Usamos AbortController.
      ...
      await apiClient.post(
        "/match/create",
        {
          ...
          isOpenSignup: parsed.isOpenSignup,
          maxPlayers: parsed.isOpenSignup ? parsed.maxPlayers : undefined,
          matchMode: form.rivalType,
        },
      ...
      const msg =
        ...
        ((err as { name?: string }).name === "CanceledError" ||
          ...
          : formatUserFacingError(err, "No se pudo crear el partido.");
  ...
  const canSubmit =
    formValid &&
    (form.isOpenSignup ? true : selectedCount > 0) &&
    !isSubmitting;
